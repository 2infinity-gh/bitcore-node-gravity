...
To develop bitcore-node-gravity:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-gravity.git
...
We now will setup symlinks in `bitcore-node-gravity` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-gravity
ln -s ~/bitcoind-rpc-gravity
```
...
cd devnode
mkdir node_modules
touch bitcore-node-gravity.json
touch package.json
```

Edit `bitcore-node-gravity.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-gravity/bin/bitcore-node-gravity start
```